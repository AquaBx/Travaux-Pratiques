...
nombre db 0,0,13;
dataLen: equ $-nombre;
etoiles: db '[', max dup('_'),'] '
etoilesLen: equ $-etoiles;
retour: db 13,10;
timeval:
	dq 0
	dq 16_666_667 ; 60hz
...
_start:
  ...
  mov       rax, 1                  ; system call for write
  mov       rdi, 1                  ; file handle 1 is stdout
  mov       rsi, etoiles             ; address of string to output
  mov       rdx, etoilesLen            ; address of string to output
  syscall                           ; invoke operating system to do the write
  ...
  mov byte [etoiles+r14], '*';
  
  mov r15, max                      ; je déplace la valeur de max dans r15 en bytes
  cmp r14,r15                       ; je compare les deux valeurs
  jle _start                        ; si r14 < r15 alors je recommence à start

  mov       rax, 1                  ; system call for write
  mov       rdi, 1                  ; file handle 1 is stdout
  mov       rsi, retour             ; address of string to output
  mov       rdx, 2                  ; address of string to output
  syscall                           ; invoke operating system to do the write
