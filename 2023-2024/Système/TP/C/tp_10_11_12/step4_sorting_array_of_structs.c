...
void sort(country_t * tab,int len ){
    ...
    do {
        permut = false;
        for (int i = 1; i < len ; i++){
            if ( tab[i-1].consumption_co2 < tab[i].consumption_co2 ){
                permut = true;
...
void sort1(country_t * tab,int len ){
    bool permut;

    do {
        permut = false;
        for (int i = 1; i < len ; i++){
            if ( tab[i-1].co2 < tab[i].co2 ){
                permut = true;
                country_t temp = tab[i];
                tab[i] = tab[i-1];
                tab[i-1] = temp;
...
void sort2(country_t * tab,int len ){
    bool permut;

    do {
        permut = false;
        for (int i = 1; i < len ; i++){
            if ( tab[i-1].cumulative_co2 < tab[i].cumulative_co2 ){
                permut = true;
                country_t temp = tab[i];
                tab[i] = tab[i-1];
                tab[i-1] = temp;
...
void main(){
   ...
   if ( !fopen_s( &file, "./owid-co2-data-excerpt.csv", "r" ) ) {
        ...
        for (int c = 0; c < 15; c++){
            ...
            country_t line = tableau[c];
            printf( "%s : Cumulative CO2 %f.\n", line.country,line.cumulative_co2 );
