...
public:
	Food(Environment* env, float x, float y, float quantite) : Agent(env, x, y), last_quantite(quantite),
	                                                           quantite(quantite)
	{
	...
	{
		float retour = quantity > getFoodQuantity() ? getFoodQuantity() : quantity;
	...
	void update() override
	{
		...
		auto color = Renderer::Color(154, 235, 38, 255);
